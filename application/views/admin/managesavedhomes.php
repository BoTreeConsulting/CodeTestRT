...
<html lang="en">
    <head>
        <meta charset="utf-8">
        <title>Realtytopia Manage Saved Homes</title>
        ...
        <script>               
            ...
            function manage_saved_search(user){
            	$('#usersearch').submit();
            }       
        </script>
    ...
    <body>   
        ...
        <div class="col-md-12 remove-padd">
            ...
            <div class="col-md-10 rtcol">
                <div class="headtext1"><i><img src=" <?php echo base_url("assets/site_images/lapicon.png");?>"></i><span>Location:</span> Admin     Panel / <span>Manage Members  / </span><span>View Saved Homes</span></div>
                <?php
                $total = $this->uri->total_segments();
                $modulename = $this->uri->segment($total);
                ?>
            	<div class="headtext1"><span> View Saved Homes</span></div>
                ...
                <div class="mtextarea mtextarea2">
        		   <div class="mtext">
        				<div style="float:right;">
        				    <form action="<?php echo base_url();?>admin/manage_saved_homes"  method="post" id="usersearch" >
                                <select id="customer" name="customer" onchange="manage_saved_search(this.value)">
                                    <option value=""> -- Select Member -- </option>
        							<?php foreach($customers as $customer){ ?>
        							<option value="<?php echo $customer['id']; ?>"  <?php if($scustomer == $customer['id']){ echo "selected";}?> ><?php echo $customer['first_name']; ?></option>
        							<?php } ?>
                                </select>
        						<button type="submit" style="display:none" value="user-search" name="user-search" class="btn btn-xs btn-default btyellow pull-right"></button>
        					</form>
                        </div>
                    </div>
                <div>
                ...
                <div class="tablewidth">
                    <div>
                		<form method="post" action="<?php echo base_url();?>admin/changesavedhomestatus">		
                		    <table id="example"  class="table-bordered table-striped table-condensed cf tablesorter tablesorter-blackice">
                				...
                				<tbody>
                    				<?php 				
                    				foreach($savedhomes as $row)
                                    {				
                    				?>
                    					<tr>						
                    						<td data-title="AgentFirstName">						
                        						<?php
                                                    $uname = ucfirst($row['first_name']).' '.ucfirst($row['last_name']);
                                                    echo $uname;  
                        						?>						
                    						</td>
                    					    <td data-title="AgentLastName"><a href="<?php echo base_url();?>home_details/index/<?php echo $row['listing_id']; ?>-<?php echo $row['pId'];?>" target="_blank">View Details</a></td>
                    						
                    						<td data-title="AgentLastName"><?php echo $row['created_on'];?></td>
                    						...
                    						<td data-title="Actions" >
                        						<a href="<?php echo base_url();?>admin/view_saved_homes/<?php echo $row['id'];?>" title="View details"><i class="icon-eye-open"></i></a>&nbsp;
                        						<a title="Delete" href="<?php echo base_url();?>admin/delete_saved_home/<?php echo $row['id'];?>" onclick="return confirm_delete('you want to delete this saved home')" ><i class="icon-trash"></i></a>
                                            </td>
