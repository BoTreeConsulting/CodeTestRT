...
<div class="col-md-12">
    ...
    <div class="headtext1"> Manage Items / <span><a style="color:#FFF" href="<?=site_url('/admin/manage_items')?>">Manage Listings</a></span></div>
    <div class="mtextarea"> 
            <!--h2>Admin Panel</h2-->
            <div>
        ...
        <form name="hot_item" id="hot_item" method="post" onSubmit="return validateForm()">
        <input name="frmSubmitR" type="hidden" id="frmSubmitR" value="yes" />
        <h3>Mark As Hot Items</h3>
           <div class="control-group" >
 <label class="control-label"  for="hot_item"><span>Mark As Hot Item&nbsp;*</span></label>
  <div class="controls">
  <select  name="hot_item" id="hot_item">
  <?php 
  for($i=0;$i<count($item_settings);$i++){
  $status=$item_settings[$i]->mark_as_hot_item;
  if($status=="Yes"){
  ?>
  <option selected="selected" value="<?php echo $item_settings[$i]->mark_as_hot_item;?>"><?php echo $item_settings[$i]->mark_as_hot_item;?></option>
