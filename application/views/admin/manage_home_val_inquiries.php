...
<html lang="en">
  <head>
    <meta charset="utf-8">
    <title>Realtytopia Manage Agent Inquiries</title>
 ...
 <?php 
  ...
  <body>
   ...
   <div class="col-lg-12 navbar-inner">
	...
	<div class="headtext1"><i><img src=" <?php echo base_url("assets/site_images/lapicon.png");?>"></i><span>Location:</span> Admin Panel / <span>Inquiry Manager / </span><span>Manage Home Value Inquiries</span></div>
     <?php
	   $total = $this->uri->total_segments();
	   $modulename = $this->uri->segment($total);
		//print_r($_SESSION);
	 ?>
    <div class="headtext1"><span>Manage Home Value Inquiries</span></div>
    ...
    <section id="no-more-tables">
		<form method="post" action="<?php echo base_url();?>admin/change_home_val_inquirystatus">
		  ...
		  <table id="example"  class="table-bordered table-striped table-condensed cf tablesorter tablesorter-blackice">
			  <thead class="cf" style=" background:#0C4371 !important;">
				...
				<tbody>
					<?php 		
					//echo "<pre>";print_r($agentsinquiries);echo "</pre>";
					foreach($home_val_inquiries as $row){				
					?>
					<tr>

                        <td data-title="AgentInquiryID" style="display:none;">                        
                        	<?php echo $row['id']; ?>                        
                        </td>

						<td data-title="AgentUserName">						
							<?php echo ucwords($row['user_fname'])." ".ucwords($row['user_lname']); ?>			
						</td>
						
						<td data-title="AgentEmail">						
							<?php echo $row['user_email']; ?>						
						</td>

                        <td data-title="AgentPhone">                        
                        	<?php echo $row['user_phone']; ?>                        
                        </td>
						<td>
							<?php 
								if($row['property_type'] == 'home_valuation'){
									echo "like a professional estimate regarding the property";
								}
								else{
									echo "Interested to sell a property with in ".$row['property_type'];
								}
							
							?>
						</td>
						<td>
							<?php echo $row['AgentFirstName']." ".$row['AgentLastName']; ?>		
						</td>
						<td data-title="AgentDate">
							<?php echo date('Y-m-d', strtotime($row['created_on']));?>						
						</td>
												
						<td data-title="Actions" >
							<a href="<?php echo base_url();?>admin/view_home_val_inquiries/<?php echo $row['id'];?>" title="View details"><i class="icon-eye-open"></i></a>&nbsp;
						
						<a title="Delete" href="<?php echo base_url();?>admin/delete_home_val_inquiries/<?php echo $row['id'];?>" onclick="return confirm_delete('you want to delete this inquiry')" ><i class="icon-trash"></i></a>
						</td>
						<td data-title="Check" >
			...
			<div align="right" style="margin-top:20px;" >
				<button type="submit" style="display:none;" name="status" value="Active" class="btn btn-xs btn-default btyellow pull" onclick="return validateForm()" >Active</button>

				<button type="submit" style="display:none;" name="status" value="Deactive" class="btn btn-xs btn-default btyellow pull" onclick="return validateForm()" >Inactive</button>
